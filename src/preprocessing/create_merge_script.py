...
print(f"  - 2025 : {len(df_2025):,} lignes")
print(f"  - 2026 : {len(df_2026):,} lignes")
...
if col_lat and col_lon:
    ...
    nb = len(df)
    df = df[df[col_lat].between(41.6, 42.1) & df[col_lon].between(-87.95, -87.5)].copy()
    print(f"  Supprimes hors Chicago : {nb - len(df):,} | Restants : {len(df):,}")
else:
    print("  ATTENTION : colonnes GPS introuvables")

print("\\n[7/8] Nettoyage dates...")
col_date = next((c for c in ["date","date_of_occurrence","occurred_on"] if c in df.columns), None)
if col_date:
    df[col_date] = pd.to_datetime(df[col_date], errors="coerce")
    nb = df[col_date].isnull().sum()
    df = df[df[col_date].notna()].copy()
    print(f"  Sans date supprimees : {nb:,}")
    ...
    print("  Colonnes temporelles creees : annee, mois, jour, heure, jour_semaine, weekend")

print("\\n[8/8] Suppression colonnes inutiles...")
a_supprimer = ["case_number","updated_on","fbi_code","beat","ward","community_area","x_coordinate","y_coordinate","location"]
...
print(f"  Liste : {list(df.columns)}")
df.to_csv(os.path.join(DOSSIER_SORTIE, "crime_clean.csv"), index=False)
print("  crime_clean.csv sauvegarde")
print("\\nPhase 5 terminee avec succes.")
'''

with open("src/preprocessing/merge_data.py", "w", encoding="utf-8") as f:
    f.write(code)

print(f"Fichier cree : {len(code)} caracteres")